...
status_t ref_group_normalization_bwd_t::execute(const exec_ctx_t &ctx) const {
    ...
    parallel_nd(C, [&](dim_t c) {
        ...
        // Accumulate parameter gradients across all batch elements
        for (dim_t n = 0; n < N; ++n) {
            size_t stat_off = n * G + g;
            float v_mean = mean[stat_off];
            float v_variance = variance[stat_off];
            float inv_std = 1.0f / sqrtf(v_variance + eps);
            ...
            for (dim_t w = 0; w < W; ++w) {
                ...
                diff_gamma += (s - v_mean) * dd * inv_std;
                diff_beta += dd;
            }
        }
        ...
        if (calculate_diff_stats) {
            ...
            for (dim_t c = 0; c < C_PER_G; ++c) {
                ...
                for (dim_t w = 0; w < W; ++w) {
                    ...
                    const size_t dd_off = DATA_OFF(diff_dst_d, n, global_c, d, h, w);

                    float s = io::load_float_value(src_d.data_type(), src, s_off);
                    float dd = io::load_float_value(diff_dst_d.data_type(), diff_dst, dd_off);

                    float dd_scaled = dd * gamma;
                    float s_normalized = (s - v_mean) * inv_std;
            ...
            for (dim_t c = 0; c < C_PER_G; ++c) {
                ...
                for (dim_t w = 0; w < W; ++w) {
                    const size_t s_off = DATA_OFF(src_d, n, global_c, d, h, w);
                    const size_t dd_off = DATA_OFF(diff_dst_d, n, global_c, d, h, w);
                    const size_t ds_off = DATA_OFF(diff_src_d, n, global_c, d, h, w);
                    ...
                    float dd_scaled = dd * gamma;
                    float s_normalized = (s - v_mean) * inv_std;
                    float v_diff_src = inv_std * (dd_scaled - mean_dd_scaled - s_normalized * mean_dd_snorm);
                    io::store_float_value(diff_src_d.data_type(), v_diff_src, diff_src, ds_off);
                }
            }
        } else {
            for (int c = get_c_start(g); c < get_c_start(g + 1); ++c) {
                ...
                for (dim_t w = 0; w < W; ++w) {
                    ...
                    float dd = io::load_float_value(diff_dst_d.data_type(), diff_dst, dd_off);
                    float v_diff_src = dd * gamma * inv_std;
                    io::store_float_value(diff_src_d.data_type(), v_diff_src, diff_src, ds_off);
                }
            }
        }
